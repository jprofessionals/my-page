...
class KtiImportService(
    ...
    private val logger = LoggerFactory.getLogger(KtiImportService::class.java)
    ...
    fun importHistoricalData(
        ...
        logger.info("=== KTI Import Started ===")
        ...
        val roundsByYear = mutableMapOf<Int, KtiRound>()
        for (year in years) {
            val existing = roundRepository.findByYear(year)
            if (existing != null) {
                roundsByYear[year] = existing
            } else {
                val round = roundRepository.save(KtiRound(
                    name = "KTI $year (importert)",
                    year = year,
                    status = KtiRoundStatus.CLOSED,
        ...
        val orgsByName = mutableMapOf<String, KtiCustomerOrganization>()
        for (orgName in orgNames) {
            val existing = organizationRepository.findByNameIgnoreCase(orgName)
            if (existing != null) {
                orgsByName[orgName] = existing
            } else {
                val org = organizationRepository.save(KtiCustomerOrganization(name = orgName))
        ...
        for (row in validRows) {
            ...
            if (existingAssignmentWithOrg != null) {
                ...
                assignment = existingAssignment ?: assignmentRepository.save(KtiAssignment(
                    round = round,
            ...
            val invitation = if (existingInvitation != null) {
                ...
                invitationRepository.save(KtiInvitation(
    ...
    private fun getOrCreateContact(org: KtiCustomerOrganization, email: String?, row: ParsedRow): KtiCustomerContact? {
        if (email == null) return null
        return contactRepository.findByEmailIgnoreCase(email)
    }

    private fun createPlaceholderContact(org: KtiCustomerOrganization, row: ParsedRow): KtiCustomerContact {
        return contactRepository.save(KtiCustomerContact(
    ...
    private fun createResponses(
        invitation: KtiInvitation,
        row: ParsedRow,
        questions: Map<String, KtiQuestion>
    ) {
        // Rating questions
