...
import ktuPublicService, {
  PublicSurveyData,
  PublicSurveyQuestion,
  SurveyResponseItem,
  SurveyAppearance,
} from '@/services/ktu-public.service'
...
export default function PublicSurveyPage() {
  ...
  useEffect(() => {
    ...
    const loadSurvey = async () => {
      try {
        const result = await ktuPublicService.getSurvey(token)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await ktuPublicService.submitResponses(token, responseList)
  ...
  if (state === 'not_found') {
    return (
      <SurveyLayout appearance={appearance}>
  ...
  return (
    <SurveyLayout appearance={appearance}>
      ...
      {appearance?.introText && (
        <div className="survey-accent-bg rounded-lg p-6 mb-8 text-center">
          <p className="text-gray-700">{appearance.introText}</p>
        </div>
      )}
...
function SurveyLayout({ children, appearance }: SurveyLayoutProps) {
  // Get theme colors with defaults - use white header for JPro logo visibility
  const headerBgColor = appearance?.headerBgColor || '#ffffff'
