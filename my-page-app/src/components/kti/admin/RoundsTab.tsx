...
export default function RoundsTab() {
  ...
  const [newRound, setNewRound] = useState<CreateKtiRound>({
    name: '',
    year: new Date().getFullYear(),
  })
  const [editRound, setEditRound] = useState<KtiRound | null>(null)
  ...
  const handleCreateRound = async () => {
    ...
    setCreating(true)
    try {
      await ktiService.createRound(newRound)
  ...
  const handleDeleteRound = async (id: number) => {
    ...
    try {
      await ktiService.deleteRound(id)
  ...
  const handleUpdateRound = async () => {
    ...
    try {
      const updateData: UpdateKtiRound = {
        name: editRound.name,
        status: editRound.status,
        openDate: editRound.openDate,
        closeDate: editRound.closeDate,
      }
      await ktiService.updateRound(editRound.id, updateData)
  ...
  const openEditModal = (round: KtiRound) => {
    setEditRound({ ...round })
    setShowEditModal(true)
  }

  const getStatusBadge = (status: KtiRoundStatus) => {
    const styles: Record<KtiRoundStatus, string> = {
  ...
  const getStatusText = (status: KtiRoundStatus) => {
    const texts: Record<KtiRoundStatus, string> = {
      DRAFT: 'Utkast',
      OPEN: 'Åpen',
      CLOSED: 'Lukket',
    }
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditRound({ ...editRound, status: e.target.value as KtiRoundStatus })}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2"
                >
