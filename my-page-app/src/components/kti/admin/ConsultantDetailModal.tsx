  ...
  const loadData = async () => {
    ...
    try {
      const [statsResponse, trendsResponse, responsesResponse] = await Promise.all([
        ktiService.getConsultantStatisticsAdmin(consultantId),
        ktiService.getConsultantTrendsAdmin(consultantId),
        ktiService.getConsultantResponsesAdmin(consultantId),
      ])
      ...
      console.error('Failed to load consultant KTI data:', error)
      toast.error('Kunne ikke laste KTI-data for konsulenten')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10">
            <div>
              <h2 className="text-xl font-semibold text-gray-900">
                KTI-resultater for {consultantName}
              </h2>
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="bg-gray-50 rounded-lg p-8 text-center text-gray-500">
                Ingen KTI-tilbakemeldinger for denne konsulenten
              </div>
            ) : (
