...
export default function SurveyImportModal({ onClose, onImportComplete }: Props) {
  const [step, setStep] = useState<Step>('upload')
  const [file, setFile] = useState<File | null>(null)
  const [preview, setPreview] = useState<KtiCsvPreview | null>(null)
  const [columnMapping, setColumnMapping] = useState<ColumnMapping>({})
  const [importing, setImporting] = useState(false)
  const [result, setResult] = useState<KtiImportResult | null>(null)
  ...
  const [, setAllUsers] = useState<KtiUser[]>([])

  const loadUsers = async () => {
    try {
      const response = await ktiService.getKtiUsers()
  ...
  const handleCreateAlias = async (aliasName: string, userId: number) => {
    ...
    try {
      const response = await ktiService.createConsultantAlias({ aliasName, userId })
      if (response.data) {
  ...
  const handleIgnoreConsultant = async (aliasName: string) => {
    setCreatingAlias(aliasName)
    try {
      const response = await ktiService.createConsultantAlias({ aliasName })
      if (response.data) {
  ...
  const handlePreview = async () => {
    ...
    try {
      await loadUsers()
      const response = await ktiService.previewCsv(file)
      if (response.data) {
  ...
  const handleValidate = async () => {
    ...
    setImporting(true)
    try {
      const response = await ktiService.importHistorical(file, true, skipUnmatched, columnMapping)
  ...
  const handleImport = async () => {
    ...
    setImporting(true)
    try {
      const response = await ktiService.importHistorical(file, false, skipUnmatched, columnMapping)
      setResult(response.data || null)
  ...
  const handleSyncUsers = async () => {
    setImporting(true)
    try {
      const response = await ktiService.syncUsersFromFlowcase()
      if (response.data) {
        toast.success(`${response.data.created ?? 0} nye, ${response.data.updated ?? 0} oppdatert`)
        await loadUsers()
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b flex justify-between items-center">
          <h2 className="text-xl font-bold">Import historiske undersøkelsesdata</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg flex justify-between items-center">
            <div>
              <div className="font-medium text-gray-900">Synkroniser konsulenter</div>
              <div className="text-sm text-gray-500">Hent oppdatert konsulentliste fra Flowcase</div>
            </div>
            <button
          ...
          {step === 'upload' && (
            <div className="space-y-4">
              <p className="text-gray-600">
                Last opp en CSV-fil med historiske KTI-data fra SurveyMonkey eller lignende.
              </p>
              <input
                type="file"
                accept=".csv"
            ...
            <div className="space-y-4">
                  ...
                  <thead className="bg-gray-100 sticky top-0">
                    <tr>
                      {preview.columns.map((col, idx) => (
                        <th key={idx} className="px-2 py-1 border-r text-left whitespace-nowrap">{col || `(${idx})`}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {preview.sampleRows.slice(0, 2).map((row, rowIdx) => (
                      <tr key={rowIdx}>
                        {preview.columns.map((_, colIdx) => (
                          <td key={colIdx} className="px-2 py-1 border-r truncate max-w-[100px]">{row[colIdx] || '-'}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  className="rounded"
                />
                <span className="text-gray-700">Hopp over konsulenter som ikke finnes</span>
              </label>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-blue-50 rounded p-3 text-center">
                  <div className="text-2xl font-bold text-blue-700">{result.importedResponses || 0}</div>
                  <div className="text-sm text-blue-600">{result.dryRun ? 'Vil importeres' : 'Importert'}</div>
                </div>
              </div>

              {/* What will be created */}
              {result.dryRun && (result.roundsToCreate?.length || result.newOrganizations || result.newContacts) && (
                <div className="bg-blue-50 rounded p-3">
                  <div className="font-medium text-blue-800 mb-1">Vil opprette:</div>
                  <ul className="text-sm text-blue-700 list-disc list-inside">
                    {(result.roundsToCreate?.length ?? 0) > 0 && <li>Undersøkelser: {result.roundsToCreate?.join(', ')}</li>}
                    {(result.newOrganizations ?? 0) > 0 && <li>{result.newOrganizations} nye kunder</li>}
                    {(result.newContacts ?? 0) > 0 && <li>{result.newContacts} nye kontaktpersoner</li>}
                  </ul>
                </div>
              )}
                    ...
                    {result.unmatchedConsultants.map((unmatched: UnmatchedConsultant) => (
                      <div key={unmatched.name} className="flex items-center justify-between bg-white rounded p-2">
                        <span className="text-sm">
                          {unmatched.name} <span className="text-gray-400">({unmatched.rowCount})</span>
                        </span>
              ...
              {result.errors && result.errors.length > 0 && (
                <div className="bg-red-50 rounded p-3">
                  <div className="font-medium text-red-800 mb-1">Feil ({result.errors.length})</div>
                  <div className="text-sm text-red-700 max-h-32 overflow-y-auto">
                    {result.errors.slice(0, 5).map((err, i) => (
                      <div key={i}>Rad {err.row}: {err.error}</div>
                    ))}
                    {result.errors.length > 5 && <div>... og {result.errors.length - 5} flere</div>}
                  </div>
                </div>
              )}
            </div>
