...
import ktiService, { KtiConsultantStatistics } from '@/services/kti.service'
...
export default function ConsultantsTab() {
  ...
  const [consultantStats, setConsultantStats] = useState<KtiConsultantStatistics[]>([])
  ...
  const loadConsultantStats = async () => {
    ...
    setLoading(true)
    try {
      const response = await ktiService.getStatisticsByConsultant(currentRound.id)
