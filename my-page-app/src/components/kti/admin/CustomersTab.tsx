...
export default function CustomersTab() {
  ...
  const [customers, setCustomers] = useState<KtiCustomerOrganization[]>([])
  const [contacts, setContacts] = useState<KtiCustomerContact[]>([])
  const [assignments, setAssignments] = useState<KtiAssignment[]>([])
  const [selectedCustomer, setSelectedCustomer] = useState<KtiCustomerOrganization | null>(null)
  ...
  // Link modal state
  const [linkContact, setLinkContact] = useState<KtiCustomerContact | null>(null)
  const [rounds, setRounds] = useState<KtiRound[]>([])
  const [consultants, setConsultants] = useState<KtiUser[]>([])
  const [selectedRoundId, setSelectedRoundId] = useState<number | null>(null)
  const [selectedConsultantId, setSelectedConsultantId] = useState<number | null>(null)

  // Form state
  const [newCustomer, setNewCustomer] = useState<CreateKtiOrganization>({ name: '' })
  const [editCustomer, setEditCustomer] = useState<KtiCustomerOrganization | null>(null)
  ...
  // Load assignments for current round
  useEffect(() => {
    if (currentRound) {
      loadAssignments()
    } else {
  ...
  const loadAssignments = async () => {
    ...
    try {
      const response = await ktiService.getAssignments(currentRound.id)
  ...
  // Check if a contact is linked to the current year's survey
  const getContactAssignment = (contactId: number): KtiAssignment | undefined => {
    return assignments.find((a) => a.contact?.id === contactId)
  }

  const loadCustomers = async () => {
    setLoading(true)
    try {
      const response = await ktiService.getOrganizations(!showInactive)
  ...
  const loadContacts = async (customerId: number) => {
    ...
    try {
      const response = await ktiService.getContacts(customerId, !showInactiveContacts)
  ...
  const handleCreateCustomer = async () => {
    ...
    setSaving(true)
    try {
      await ktiService.createOrganization(newCustomer)
  ...
  const handleUpdateCustomer = async () => {
    ...
    setSaving(true)
    try {
      const updateData: UpdateKtiOrganization = {
        name: editCustomer.name,
        organizationNumber: editCustomer.organizationNumber,
        active: editCustomer.active,
      }
      await ktiService.updateOrganization(editCustomer.id, updateData)
  ...
  const openEditCustomer = (customer: KtiCustomerOrganization) => {
    setEditCustomer({ ...customer })
    setShowEditCustomerModal(true)
  }

  const openEditContact = (contact: KtiCustomerContact) => {
    setEditContact({ ...contact })
    setShowEditContactModal(true)
  }

  const handleDeactivateCustomer = async (customer: KtiCustomerOrganization) => {
    if (!confirm(`Er du sikker på at du vil deaktivere "${customer.name}"? Kunden vil ikke bli brukt i nye undersøkelser.`)) return
    setSaving(true)
    try {
      await ktiService.updateOrganization(customer.id, {
        name: customer.name,
  ...
  const handleDeactivateContact = async (contact: KtiCustomerContact) => {
    ...
    setSaving(true)
    try {
      await ktiService.updateContact(contact.id, {
        name: contact.name,
        email: contact.email,
        phone: contact.phone,
        title: contact.title,
  ...
  const openLinkModal = async (contact: KtiCustomerContact) => {
    setLinkContact(contact)
    // Pre-select current round if available
  ...
  const handleCreateLink = async () => {
    ...
    setSaving(true)
    try {
      await ktiService.createAssignment(selectedRoundId, {
              ...
              {loadingContacts ? (
                ...
                <div className="space-y-3">
                  {contacts.map((contact) => {
                    const assignment = getContactAssignment(contact.id)
                    return (
                    <div
                      key={contact.id}
                      className={`p-4 border rounded-lg ${
                        !contact.active || contact.optedOut ? 'bg-gray-50 opacity-60' : assignment ? 'bg-green-50 border-green-200' : 'bg-white'
                      }`}
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {/* Show linked consultant for current year */}
                          {assignment && (
                            <div className="mt-2 flex items-center gap-1 text-xs text-green-700">
                              ...
                              <span>
                                {selectedYear}: {assignment.consultant?.name || 'Ukjent konsulent'}
                              </span>
                            </div>
                          )}
                        </div>
