...
function Kalkulator() {
  ...
  const [antallTimerFakturert, setAntallTimerFakturert] = useState(
    +(antallArbeidsdager * 7.5).toFixed(2),
  )
  ...
  function loadScenarioHigh() {
    loadScenarioAvg()
    setTimeprisProsjekt(
      round(parseInt(getSetting(settings, 'CALC_TIMEPRIS') ?? '0') * 1.2),
    )
  }
  ...
  function loadScenarioLow() {
    loadScenarioAvg()
    setTimeprisProsjekt(
      round(parseInt(getSetting(settings, 'CALC_TIMEPRIS') ?? '0') * 0.8),
    )
  }
  ...
  function TilgjengeligTid() {
    return Math.max(
      Math.min(
        DinNormerteTid() - AntallTimerBetaltTid(),
        +antallTimerInterntid + AntallTimerUbetaltKompetanse(),
      ),
      0,
    )
  }
  ...
  function SumBetaltKompetanse() {
    return timeprisKompetanse * 0.52 * AntallTimerBetaltKompetanse()
  }

  function SumBonus() {
    return (
      +foredragsbonus +
      (salgsbonus === true ? salgsbonusBelop : 0) +
      (rekrutteringsbonus === true ? rekrutteringsbonusBelop : 0)
    )
  }

  function AntallTimerBetaltKompetanse() {
    return Math.min(+antallTimerKompetanse, +restKompetanseBudsjett)
  }

  function AntallTimerUbetaltKompetanse() {
    return Math.max(antallTimerKompetanse - AntallTimerBetaltKompetanse(), 0)
  }
  ...
  function BruttoAarsLonnFakturert() {
    return (
      ((billableHoursPerYear * timeprisProsjekt * stillingsprosent) / 100) *
      0.52
    )
  }

  const handleAntallArbeidsdagerChange = (
  ...
  const handleStillingsprosentChange = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setStillingsprosent(Math.min(Math.floor(Number(e.target.value)), 100))
  }

  const handleForrigeStillingsprosentChange = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setForrigeStillingsprosent(
      Math.min(Math.floor(Number(e.target.value)), 100),
    )
  }
  ...
  const handleForedragsbonusChange = (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    ...
    <>
      <div className="flex flex-col gap-4 p-4">
        <div className="prose max-w-fit">
          ...
          <p>
            Her kan du se hvordan vi beregner lønn hver måned. Du kan selv leke
            med tallene for å se hvordan dette påvirker beregningen av
            bruttolønn. Du kan også klikke på de predefinerte
            <strong> eksemplene</strong> under for å se hvordan beregningen blir
            ...
            <AccordionContent className="p-2 rounded-b-lg data-open:border-2">
              <div className="flex flex-col gap-2 items-start">
                <div className="space-x-2">
                  ...
                  <span>- Gjennomsnittlig timepris I JPro</span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>
                    - 20% høyere timepris enn dagens gjennomsnitt i JPro
                  </span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>
                    - 20% lavere timepris enn dagens gjennomsnitt i JPro
                  </span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>- Lønn om man ikke har oppdrag</span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>- Lønn om man går fra 100% til 50% stilling</span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>- Lønn om man har en 50% stilling</span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>- Lønn om man går fra 50% til 100% stilling</span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>- Lønn første måned som ansatt i JPro</span>
                </div>
                <div className="space-x-2">
                  <button
                  ...
                  <span>- Lønn måneden etter fratredelse</span>
                </div>
              </div>
            </AccordionContent>
          </AccordionItem>
        ...
        <div className="flex flex-wrap gap-6 items-start">
          ...
          <div className="rounded-lg border border-gray-300 border-solid grow shrink-0">
            <div className="p-4 border-b border-gray-300 bg-slate-200">
              Basis
            </div>
            <div className="gap-2 p-4 form-control calculator-group">
              ...
              <label className="input-group gap-1">
                <span>
                  Antall arbeidsdager
                  <ReadMoreIcon text="Antall arbeidsdager i foregående måneden. I snitt er det 21,67 arbeidsdager i én måned om man ikke tar hensyn til ferie og helligdager." />
                </span>
                <input
                  type="number"
                  min="0"
              ...
              <label className="input-group gap-1">
                <span>
                  Stilling (% forrige)
                  <ReadMoreIcon text="Stillingsprosent i foregående måned, denne påvirker beregningen av minimumslønn og estimat for utbetalt forskudd." />
                </span>
                <input
                  type="number"
                  min="0"
              ...
              <label className="input-group gap-1">
                <span>
                  Stilling (% nå)
                  <ReadMoreIconPushRight text="Stillingsprosent i inneværende måned, denne påvirker kun forskuddslønnen." />
                </span>
                <input
                  type="number"
                  min="0"
          ...
          <div className="rounded-lg border border-gray-300 border-solid grow shrink-0">
            ...
            <div className="gap-2 p-4 form-control calculator-group">
              ...
              <label className="input-group gap-1">
                <span>
                  Annen bonus
                  <ReadMoreIconPushRight text="F.eks. bonus for å holde foredrag internt eller eksternt. Beløp anvhenger av flere faktorer, som lenger hvor foredraget holdes osv. se intranett for mer detaljer" />
                </span>
                <input
                  type="number"
                  min="0"
            ...
            <ul className="flex flex-col gap-2 justify-between p-4">
              ...
              <li className="flex justify-between gap-4 ml-4">
                <span className="flex justify-between gap-1">
                  Utbetalt forskudd
                  <ReadMoreIcon
                    text={`Trekk for forskudd utbetalt foregående måned, vil normalt være Minimumslønn * Stillingsprosent => ${getInNok(garantilonn)} * ${getAsNo(forrigeStillingsprosent / 100)}`}
                  />
                </span>
                <span className="text-red-500">
                  {getInNok(UtbetaltForskudd())}
                </span>
              </li>
              ...
              <li className="flex justify-between gap-4 ml-4">
                <span className="flex justify-between gap-1">Bonus</span>
                <span>{getInNok(SumBonus())}</span>
              </li>
              <li className="flex justify-between gap-4 ml-4">
                <span className="flex justify-between gap-1">Bruttotrekk</span>
