...
export const getVacantApartmentsOnDay = (
  ...
  const vacantApartmentsInPeriod = getVacantApartments(
    selectedDate,
    vacancies,
    cutOffDateVacancies,
    dateFormat,
...
export const getInfoNotices = (
  date: string,
  allInfoNotices: InfoBooking[],
): InfoBooking[] => {
  return (
...
export const getInfoNoticeVacancyOnGivenDay = (
  selectedDate: Date,
...
export const sortVacantApartment = (apartments: Apartment[]) => {
  apartments.sort(
    (a, b) =>
      cabinOrder.indexOf(a.cabin_name) - cabinOrder.indexOf(b.cabin_name),
  )
  return apartments
}
