...
import { getInfoNoticesOnDay } from '@/components/hyttebooking/month-overview/components/month-calendar/monthCalendarUtil'
...
export default function MonthOverview() {
  ...
  const [vacancyLoadingStatus, setVacancyLoadingStatus] =
    useState<LoadingStatus>(LoadingStatus.init)
  ...
  const { data: yourPendingBookings, refetch: refetchYourPendingBookings } =
    useQuery<Booking[]>({
      ...
      queryFn: async () => {
        const yourPendingBookings: Booking[] =
          await ApiService.getPendingBookingsForUser()
  ...
  const { data: allInfoNotices } = useQuery<InfoBooking[]>({
    ...
    queryFn: async () => {
      const fetchedInfoNotices: InfoBooking[] = await ApiService.getInfoNotices(
        startDateBookings,
        endDateBookings,
      )
  ...
  const { data: allPendingBookingTrains, refetch: refetchPendingBookings } =
    useQuery({
      ...
      queryFn: async () => {
        const allPendingBookingTrainsForAllApartments: PendingBookingTrain[] =
  ...
  useEffect(() => {
    if (
      vacancyLoadingStatus === LoadingStatus.init &&
      userFetchStatus === 'fetched'
    ) {
      refreshVacancies()
      getAllApartments()
    }
  ...
  useEffect(() => {
    if (
      infoNoticeVacancyLoadingStatus === LoadingStatus.init &&
      !!user?.admin
    ) {
      fetchAllUsers()
  ...
  const handleOpenPendingBookingDeleteModal = (
    pendingBooking: number | null,
  ) => {
  ...
  const getInfoNoticesOnDay = (date: Date) => {
    ...
    const infoNoticesOnDay = all.filter(
      (infoNotice) =>
        !isBefore(date, infoNotice.startDate) &&
        !isAfter(date, infoNotice.endDate),
    )
  ...
  const getInfoNoticeVacancyOnDay = (selectedDate: Date) => {
    const vacant = getInfoNoticeVacancyOnGivenDay(
      selectedDate,
      infoNoticeVacancies,
    )
        ...
        <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8 prose">
          <Tabs defaultValue="oversikt" className="">
                ...
                <div>
                    ...
                    <div>
                      ...
                      {infoNotices.map((infoNotice, index) => (
                        <InfoNoticeItem
                          ...
                          infoNoticeDeleteModalIsOpen={
                            infoNoticeDeleteModalIsOpen
                          }
                          infoNoticeVacancies={infoNoticeVacancies}
                          onAddInfoNoticeClick={handleAddInfoNoticeClick}
                          onCloseInfoNoticeDeleteModal={
                            handleCloseInfoNoticeDeleteModal
                          }
                          onConfirmInfoNoticeDelete={
                            handleConfirmInfoNoticeDelete
                          }
                          onEditInfoNotice={handleEditInfoNotice}
                          onInfoNoticeDeleteModal={
                            handleOpenInfoNoticeDeleteModal
                          }
                          date={date}
                  ...
                  {bookingItems.length === 0 ? (
                    ...
                    <div>
                      ...
                      {sortVacantApartment(vacantApartmentsOnDay).map(
                        (apartment, index) => (
                          <div key={index}>
                            <div
            ...
            <TabsContent value="trekning">
              ...
              {sortPendingBookings(pendingBookingsOnDay).map(
                (pendingBooking, index) => (
                  <div key={index}>
                    <div
                      className={`mt-1 mb-1 ${
