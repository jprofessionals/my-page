...
const DrawPeriod = ({ drawingPeriod, dateFormat }: Props) => {
    ...
    const manualDrawingNotice = (autoDrawingDate >= earliestStartDate) ? 'Oppholdet starter før automatisk trekningsdato. Kontakt Roger for manuell trekning' : '';

    return (
        <>
            {drawingPeriod.pendingBookings[0].apartment.cabin_name}{' '}
            fra{' '}
            {format(new Date(drawingPeriod.startDate), dateFormat)}{' '}
            til{' '}
            {format(new Date(drawingPeriod.endDate), dateFormat)}.{' '}
            Planlagt trekning{' '}
