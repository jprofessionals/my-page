...
const BookingItem = ({bookingItemsIndex, booking, yourBookings, bookingItems, admin, onEditBooking, onOpenDeleteModal, onCloseModal, onConfirmDelete, deleteModalIsOpen, showEditFormForBooking, apartments, cutOffDateVacancies, onRefreshVacancies}: Props) => {
    ...
    const isYourBooking = !booking.isPending && yourBookings?.some((yourBooking) => yourBooking.id === booking.id) || booking.isPending;
    const prevCabinName = bookingItemsIndex > 0 ? bookingItems[bookingItemsIndex - 1].apartment.cabin_name : null;
    ...
    return (
        <div key={booking.id}>
            ...
            <div className={`mt-2 mb-1 pl-2 flex ${cabinBorderColorClasses[currentCabinName]} border-l-2`}>
                {isYourBooking || admin ? (
