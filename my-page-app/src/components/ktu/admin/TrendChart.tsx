...
interface TrendChartProps {
  ...
  maxValue?: number
  valueSuffix?: string
  showScaleHint?: boolean // Show "1=svakest, 6=best" hint
}
...
export default function TrendChart({
  ...
  const formatValue = (val: number) => {
    if (Number.isInteger(val)) return `${val}${valueSuffix}`
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ maxHeight: height }}>
        ...
        {showDots &&
          validData.map((d) => {
            const x = xScale(d.year)
            const y = yScale(d.value)
            const originalPoint = data.find((p) => p.year === d.year)
            const responseCount = originalPoint?.responseCount
            return (
              <g key={d.year}>
