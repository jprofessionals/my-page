...
type FilterStatus = 'all' | KtuRoundStatus

export default function AssignmentsTab() {
  const [surveys, setSurveys] = useState<KtuRound[]>([])
  ...
  const loadSurveyDetails = async (surveyId: number) => {
    ...
    try {
      const [assignmentsRes, invitationsRes, responsesRes] = await Promise.all([
        ktuService.getAssignments(surveyId),
        ktuService.getInvitations(surveyId),
        ktuService.getRoundResponses(surveyId),
      ])
  ...
  const handleSendInvitations = async () => {
    ...
    try {
      const result = await ktuService.sendInvitations(selectedSurvey.id)
      if (result.data) {
  ...
  const handleSendReminders = async () => {
    ...
    try {
      const result = await ktuService.sendReminders(selectedSurvey.id)
      if (result.data) {
  ...
  // Edit invitation
  const openEditInvitation = (invitation: KtuInvitation) => {
  ...
  const handleUpdateInvitation = async () => {
    ...
    setUpdating(true)
    try {
      await ktuService.updateInvitation(editingInvitation.id, {
  ...
  const handleDeleteResponse = async (responseId: number) => {
    if (!confirm('Er du sikker pa at du vil slette dette svaret?')) return
  ...
  const isRoundClosed = selectedSurvey?.status === 'CLOSED'
      ...
      <div className="flex-1">
        {selectedSurvey ? (
          <div className="space-y-6">
            ...
            {/* View mode toggle and Assignment Grid / Table */}
            <div className="bg-white rounded-lg shadow">
              <div className="p-4 border-b flex items-center justify-between">
                <h3 className="text-lg font-semibold">Tildelinger</h3>
              ...
              {loadingDetails ? (
                ...
                <div className="p-4">
                  <AssignmentGrid
                    roundId={selectedSurvey.id}
                    assignments={assignments}
                    invitations={invitations}
                    consultants={consultants}
                    organizations={organizations}
                    contacts={contacts}
                    onDataChange={() => loadSurveyDetails(selectedSurvey.id)}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="OPENED">Apnet</option>
                  <option value="RESPONDED">Besvart</option>
                  <option value="EXPIRED">Utlopt</option>
                </select>
              </div>
            </div>
