    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      {hasTextOverride && !disabled && (
        <div className="mb-3 p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-700 flex items-start gap-2">
          ...
          <span>Endret spørsmålstekst kan gjøre det vanskeligere å sammenligne resultater med tidligere år.</span>
        </div>
      )}
      ...
      {!disabled && (
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-sm text-gray-600">
            Obligatorisk
            {requiredOverride !== null && requiredOverride !== undefined && requiredOverride !== question.required && (
              <span className="text-amber-600 ml-1">(endret fra standard)</span>
            )}
          </span>
        </div>
      )}
