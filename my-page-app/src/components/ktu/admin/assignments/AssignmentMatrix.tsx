...
export default function AssignmentMatrix({
  ...
  const contactHasSentInvitations = useCallback((contactId: number) => {
    return invitations.some(inv => {
      const assignment = assignments.find(a => a.id === inv.assignmentId)
      return assignment?.contact.id === contactId &&
  ...
  const visibleConsultants = useMemo(() => {
    const visibleIds = new Set([...consultantIdsWithAssignments, ...manuallyAddedIds])
  ...
  }, [allConsultants, consultantIdsWithAssignments, manuallyAddedIds, searchQuery])
  ...
  const handleToggle = useCallback(async (consultant: KtuConsultant, contact: KtuCustomerContact) => {
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        <div ref={searchRef} className="relative flex-1 max-w-xs">
          ...
          {showDropdown && searchQuery.trim() !== '' && availableToAdd.length > 0 && (
                  ...
                  >
                    <div className="flex flex-col items-center">
                      <div className="flex items-center gap-1">
                        <span className="truncate max-w-[100px]" title={contact.name}>
                          {contact.name}
                        </span>
                        {!disabled && (
                          <button
                      ...
                      <div className="text-gray-400 font-normal truncate text-[10px] max-w-[110px]" title={contact.email || ''}>
                        {contact.email || <span className="italic">ingen e-post</span>}
                      </div>
                    </div>
                ...
                return (
                  <tr key={consultant.id} className="hover:bg-blue-50/30">
                    ...
                    {contacts.map((contact) => {
                      const cellKey = getCellKey(consultant.id, contact.id)
                      const assignment = getAssignment(consultant.id, contact.id)
                      const invitation = assignment ? getInvitation(assignment.id) : undefined
                      ...
                      const isLocked = invitation && ['SENT', 'OPENED', 'RESPONDED', 'EXPIRED'].includes(invitation.status)
                      ...
                      return (
                        <td
                          ...
                          className={`px-2 py-2 text-center border-b ${
                            hasError ? 'bg-red-50' : isLocked ? 'bg-gray-50' : ''
                          }`}
                        >
                          <label
                            className={`inline-flex items-center justify-center ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                            title={isLocked ? 'Kan ikke endres - invitasjon er sendt' : undefined}
                          >
                            {isLoading ? (
                              ...
                              <>
                                <input
                                  type="checkbox"
                                  checked={!!assignment}
                                  onChange={() => handleToggle(consultant, contact)}
                                  disabled={isDisabled}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          ...
          >
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {contactHasSentInvitations(activeContactMenu) ? 'Deaktiver' : 'Fjern'}
              </button>
            </div>
          </div>
        </div>
