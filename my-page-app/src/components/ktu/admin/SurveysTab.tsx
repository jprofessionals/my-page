...
interface Props {
  onSelectSurvey?: (survey: KtuRound) => void
}
...
export default function SurveysTab({ onSelectSurvey }: Props) {
  ...
  const [newSurvey, setNewSurvey] = useState<CreateKtuRound>({
    name: '',
    year: new Date().getFullYear(),
  })
  const [editingSurvey, setEditingSurvey] = useState<KtuRound | null>(null)
  const [editForm, setEditForm] = useState<UpdateKtuRound>({ name: '' })
  ...
  const handleCreateSurvey = async () => {
    ...
    setCreating(true)
    try {
      await ktuService.createRound(newSurvey)
  ...
  const handleEditSurvey = (survey: KtuRound, e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  const handleUpdateSurvey = async () => {
    ...
    setUpdating(true)
    try {
      await ktuService.updateRound(editingSurvey.id, editForm)
  ...
  const handleDeleteSurvey = async (survey: KtuRound, e: React.MouseEvent) => {
    ...
    try {
      await ktuService.deleteRound(survey.id)
