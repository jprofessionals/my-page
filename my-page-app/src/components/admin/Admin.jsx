...
function Admin() {
    ...
    const handleExpandUser = (user, event) => {
        ...
        const expandedArea = document.querySelector(`[data-expanded-user="${user.email}"]`);
        ...
        return (
            <>
                <div className="admin-container">
                    ...
                    <div className="mb-3">
                        <label htmlFor="filterInput" className="form-label">Filtrer brukere:</label>
                        <input type="text" className="form-control" id="filterInput"
                               onChange={(e) => setFilterValue(e.target.value)} style={{width: '300px'}}/>
                    </div>
                    ...
                    <Table striped bordered hover style={!isLoading && budgetTypes.length > 0 ? {} : {display: "none"}}>
                        ...
                        <tbody>
                        {users
                            .filter(user => user.name.toLowerCase().includes(filterValue.toLowerCase())) // Filter users by text input value
